...
int optimality_check_parallel(int n, vector<bool>& cov_row, vector<int> ass_row){
    int match_count = 0;
...
void reset_arrays(int n, vector<bool>& cov_row, vector<bool>& cov_col, vector<int>& prv_row, vector<int>& prv_col, vector<int>& suc_row, vector<int>& suc_col, vector<double>& slack){
    ...
    for (int j=0; j<n; j++){
        if (cov_col[j] == 0){
            ...
            for (int x=0; x < f_in.size(); x++){
                ...
                // Get row to which is assigned to j
                int i_new = ass_col[j];
                if (slack[j] == 0){
...
void update_pass(int n, vector<bool>& active, vector<bool> cov_row, vector<bool> cov_col, vector<double>& dual_row, vector<double>& dual_col, vector<double>& slack, vector<int> prv_col){
	...
	double theta = inf;
    #pragma omp parallel for reduction(min: theta) num_threads(NUM_THREADS)
    for (int j=0; j<n; j++){
        if (slack[j] > 0){
...
int hungarian_parallel(vector<vector<int>> &cost, int n){
    // Init variables
	int ret = 0; //weight of the optimal matching
    vector<int> ass_row(n, -1); //row assignment
    vector<int> ass_col(n, -1); //col assignment
    vector<bool> cov_row(n, 0); //row cover
    vector<bool> cov_col(n, 0); //col cover
	...
	init_potentials_parallel(cost, n, dual_row, dual_col);
    while (true){
        if(check_flag){
            //reset Cover Arrays and Predecessor Arrays to initial state
			reset_arrays(n, cov_row, cov_col, prv_row, prv_col, suc_row, suc_col, slack);

            int match_count = optimality_check_parallel(n, cov_row, ass_row);
...
int main(int argc, char*argv[])
{
	...
	out_file_all<<"Hungarian Parallel OpenMP ("<<NUM_THREADS<<" Threads): \n";
	out_file_all<<"Size\t\t\t\tCost\t\t\t\tTotal Time in s\t\t\tTotal CPU time in ms\n";
	
	ofstream out_file_mean;
    out_file_mean.open (filename_mean_runs);
	out_file_mean<<"Hungarian Parallel OpenMP ("<<NUM_THREADS<<" Threads, "<<num_test_runs<<" runs): \n";
